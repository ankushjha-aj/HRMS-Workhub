...
interface PunchWidgetProps {
    ...
    date?: Date;
    userFaceDescriptor?: any;
    userFaceEnrolled?: boolean;
}

export default function PunchWidget({ userId, initialState, date, userFaceDescriptor, userFaceEnrolled }: PunchWidgetProps) {
    ...
    const [pendingAction, setPendingAction] = useState<{ action: 'IN' | 'OUT' | 'START_BREAK' | 'END_BREAK', type?: 'LUNCH' | 'TEA' } | null>(null);

    // Sync state when initialState prop changes (e.g., when date changes)
    useEffect(() => {
    ...
    // Check if current date is Sunday (0 = Sunday)
    const isSunday = (date || new Date()).getDay() === 0;
    ...
    const handlePunch = async (action: 'IN' | 'OUT' | 'START_BREAK' | 'END_BREAK', type?: 'LUNCH' | 'TEA') => {
        if (loading) return;
        if (isSunday) return; // Block all actions on Sunday
        if (action === 'OUT' && !canPunchOut) return;

        // Check if face enrolled and show verification
        if (userFaceEnrolled && userFaceDescriptor) {
            // Store pending action and show verification modal
            setPendingAction({ action, type });
            setShowVerification(true);
        } else {
            // No face enrolled, proceed directly (shouldn't happen in production)
            executePunch(action, type);
        }
    };

    const executePunch = async (action: 'IN' | 'OUT' | 'START_BREAK' | 'END_BREAK', type?: 'LUNCH' | 'TEA') => {
        setLoading(true);
        try {
    ...
    const onVerificationSuccess = () => {
        ...
        if (pendingAction) {
            executePunch(pendingAction.action, pendingAction.type);
    ...
    // Disabled Day Completed card - keeping same interface after punch out
    if (false && state.status === 'PUNCHED_OUT') {
        ...
        <>
            <div className="bg-employee-surface-light dark:bg-employee-surface-dark rounded-xl p-6 md:p-8 border border-employee-border-light dark:border-employee-border-dark shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group min-h-[240px]">
                ...
                <div className={`flex flex-col gap-2 z-10 transition-all duration-500 ${state.status !== 'NOT_PUNCHED' ? 'md:w-1/2' : 'w-full md:w-auto'}`}>
                    <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm font-bold mb-1">
                    ...
                    {state.status === 'NOT_PUNCHED' ? (
                        ...
                        <div className="flex flex-col gap-1">
                            <div className="flex items-baseline gap-2 font-mono text-xl sm:text-2xl font-bold tracking-tighter tabular-nums text-green-600 dark:text-green-400">
                                {(() => {
                                    const h = Math.floor(state.totalWorkSeconds / 3600);
                                    const m = Math.floor((state.totalWorkSeconds % 3600) / 60);
                                    const s = state.totalWorkSeconds % 60;
                            ...
                            <div className="text-[10px] text-green-600/70 dark:text-green-400/70 font-semibold uppercase tracking-wider">
                                Total Working Hours
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                        <div className="flex flex-wrap gap-2">
                            ...
                            {state.status !== 'NOT_PUNCHED' && (
                                <>
                                    <div className="text-sm font-semibold px-2.5 py-1.5 rounded-full w-fit flex items-center gap-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800/50">
                                        <span className="material-symbols-outlined text-[16px]">login</span>
                                        In: {formatTime(state.punchIn)}
                                    </div>
                                    {/* Punch Out Badge - Shows when user has punched out */}
                                    {state.punchOut && (
                                        <div className="text-sm font-semibold px-2.5 py-1.5 rounded-full w-fit flex items-center gap-1.5 bg-red-100/60 dark:bg-red-900/20 text-red-600/70 dark:text-red-400/60 border border-red-200/50 dark:border-red-800/30">
                                            <span className="material-symbols-outlined text-[16px]">logout</span>
                                            Out: {formatTime(state.punchOut)}
                                        </div>
                                    )}
                        ...
                        {state.status !== 'NOT_PUNCHED' && state.status !== 'PUNCHED_OUT' && (
                            <div className="flex gap-3 text-xs text-slate-500 dark:text-slate-400 font-bold ml-1">
                ...
                <div className="z-10 flex gap-4 items-center justify-end flex-1 w-full md:w-auto">
                    {state.status === 'NOT_PUNCHED' && (
                        <button
                            ...
                            className={`flex flex-col items-center justify-center size-36 sm:size-40 rounded-full transition-all shadow-lg active:scale-95 group/btn ${isSunday
                                ? 'bg-slate-300 dark:bg-slate-700 text-slate-500 dark:text-slate-400 cursor-not-allowed'
                                : 'bg-employee-primary hover:bg-employee-primary-hover text-white hover:shadow-employee-primary/30 hover:scale-105'
                                }`}
                        >
                            {loading ? (
                                <span className="size-8 border-2 border-white/30 border-t-white rounded-full animate-spin mb-1"></span>
                            ) : (
                                <span className="material-symbols-outlined text-4xl mb-1 group-hover/btn:scale-110 transition-transform">
                                    {isSunday ? 'block' : 'fingerprint'}
                                </span>
                            )}
                            <span className="text-lg font-bold tracking-tight">{isSunday ? 'Sunday' : 'Punch In'}</span>
                            <span className="text-xs font-normal opacity-80 mt-1">{isSunday ? 'Locked' : 'Start your day'}</span>
                        </button>
                    )}

                    {(state.status === 'PUNCHED_IN' || state.status === 'ON_BREAK' || state.status === 'PUNCHED_OUT') && (
                        <div className="flex items-center gap-4 animate-in slide-in-from-right-10 fade-in duration-500">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <button
                                    ...
                                    disabled={loading || isSunday || (state.status !== 'ON_BREAK' && (!canLunch || state.totalLunchSeconds > 0)) || (state.status === 'ON_BREAK' && state.breakType !== 'LUNCH') || state.status === 'PUNCHED_OUT'}
                                    ...
                                    ${state.status === 'ON_BREAK' && state.breakType === 'LUNCH'
                                            ...
                                            : state.status === 'PUNCHED_OUT'
                                                ? 'bg-amber-300/40 dark:bg-amber-900/30 text-amber-600/80 dark:text-amber-400/60 border-amber-200/50 dark:border-amber-800/30 cursor-not-allowed opacity-70'
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        ...
                                        {state.status === 'PUNCHED_OUT' && <span className="block text-sm font-semibold leading-none mt-0.5">{formatDuration(state.totalLunchSeconds)}</span>}
                                        {state.status === 'ON_BREAK' && state.breakType === 'LUNCH' && <span className="block text-[10px] leading-none">End Lunch</span>}
                                        {state.status !== 'ON_BREAK' && state.status !== 'PUNCHED_OUT' && <span className="block text-[10px] leading-none">{state.totalLunchSeconds > 0 ? 'Completed' : (canLunch ? 'Start' : '12:00 PM')}</span>}
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    disabled={loading || isSunday || (state.status !== 'ON_BREAK' && (!canTea || state.totalTeaSeconds > 0)) || (state.status === 'ON_BREAK' && state.breakType !== 'TEA') || state.status === 'PUNCHED_OUT'}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all min-w-[140px]
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        ...
                                        {state.status === 'PUNCHED_OUT' && <span className="block text-sm font-semibold leading-none mt-0.5">{formatDuration(state.totalTeaSeconds)}</span>}
                                        {state.status === 'ON_BREAK' && state.breakType === 'TEA' && <span className="block text-[10px] leading-none">End Tea</span>}
                                        {state.status !== 'ON_BREAK' && state.status !== 'PUNCHED_OUT' && <span className="block text-[10px] leading-none">{state.totalTeaSeconds > 0 ? 'Completed' : (canTea ? 'Start' : '05:00 PM')}</span>}
                                    </div>
                                </button>
                            </div>
                            ...
                            <button
                                ...
                                disabled={loading || isSunday || !canPunchOut || state.status === 'ON_BREAK' || !!state.punchOut}
                                ...
                                ${!canPunchOut || state.status === 'ON_BREAK' || state.punchOut
                                        ? 'bg-red-300/40 dark:bg-red-900/30 text-red-400/70 dark:text-red-400/50 cursor-not-allowed shadow-none opacity-70'
            ...
            {
                showVerification && userFaceDescriptor && (
                    <FaceVerification
                        userId={userId}
